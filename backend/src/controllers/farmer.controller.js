...
export const getCropsByFarmerIdHandler = asyncHandler(async (req, res) => {
    const crops = await getCropsByFarmerId(req.params.id);

    return res.status(200).json({
        message: "Farmer's crops fetched successfully",
        crops,
    });
});
